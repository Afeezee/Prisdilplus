...
// Now powered by live PostgreSQL data via /api/admin/data
// ============================================================

// ─── Data Cache ────────────────────────────────────────────

let _cache = { players: [], sessions: [], timestamp: 0 };
const CACHE_TTL = 30_000; // 30 seconds

/**
 * Fetch live data from the database via the admin API.
 * Caches results for CACHE_TTL ms to avoid hammering the server on tab switches.
 */
export async function fetchAdminData() {
  const now = Date.now();
  if (_cache.timestamp && now - _cache.timestamp < CACHE_TTL && _cache.players.length + _cache.sessions.length > 0) {
    return _cache;
  }
...
function computePlayerMetrics(player) {
  ...
  const nicenessScore = cooperationRate;
...
// ─── Get All Players (async — fetches from DB) ─────────────

export async function getAllPlayers() {
  const { players } = await fetchAdminData();
...
// ─── Get All Sessions (async — fetches from DB) ────────────

export async function getAllSessions() {
  const { sessions } = await fetchAdminData();
...
export async function getCooperationOverTime() {
  const sessions = await getAllSessions();
...
export async function getMoveFrequencies() {
  const sessions = await getAllSessions();
...
export async function getRoundPositionAnalysis() {
  const sessions = await getAllSessions();
  const roundData = {};
...
export async function getFirstMoveAnalysis() {
  const sessions = await getAllSessions();
  let cooperate = 0;
  let defect = 0;
...
export async function getRetaliationLatency() {
  const sessions = await getAllSessions();
...
export async function getProfileBreakdown() {
  const players = await getAllPlayers();
  const groups = {};
...
export async function getAIStrategyImpact() {
  const sessions = await getAllSessions();
  const strategyData = {};
...
export async function getPlayerSessions(alias) {
  const sessions = await getAllSessions();
...
export async function exportPlayerSummaryDataset() {
  const players = await getAllPlayers();
...
export async function exportGameSessionsDataset() {
  const sessions = await getAllSessions();
...
export async function exportRoundByRoundDataset() {
  const sessions = await getAllSessions();
...
export async function exportBehaviouralMetricsMatrix() {
  const players = await getAllPlayers();
...
export async function exportFullRawExport() {
  const players = await getAllPlayers();
  const sessions = await getAllSessions();
